...
from utils.selenium_utils import initialise_chrome_driver, get_element_by_xpath
...
def login_to_company_check(
    driver,
    email=os.getenv("COMPANYCHECKEMAIL"),
    password=os.getenv("COMPANYCHECKEMAIL")):
    ...
    login_button = driver.find_element_by_xpath("//a[@class='log-in-link']")
    # login_button.click()
    driver.execute_script("arguments[0].click();", login_button)

    print ("finding username")
    username_field = get_element_by_xpath(driver, "//*/input[@class='login-form__email']")

    # username_field.send_keys(email)
    # driver.execute_script(f"document.getElementsByClassName('login-form__email')[0].value='{email}'")
    driver.execute_script("arguments[0].setAttribute('value', '" + email + "');", username_field)

    print ("finding password")
    password_field = get_element_by_xpath(driver, "//*/input[@class='login-form__password']")

    # password_field.send_keys(password)
    driver.execute_script("arguments[0].setAttribute('value', '" + password + "');", password_field)
...
def scrape_company_check(link):
    ...
    if status_code == 200:
        ...
        if table is not None:
            ...
            # directors_table.to_csv("test.csv")
            # print (directors_table)
            # raise Exception
            
            # find directors
            # directors_table = directors_table.loc[directors_table["Role"]=="Director"]
            
            # directors = {
            #     f"director_{i+1}": name 
            #     for i, name in enumerate(directors_table["Name"])
            # }
...
def main():
    ...
    if driver is None:
        ...
        login_to_company_check(driver)


    # output_dir = "member_summaries"

    # membership_level = "Freemium"
    ...
    #     company_check_name, address, link = search_company_check(company_name, postcode, check_postcode=False)

    #     company_data_from_company_check = pd.Series(
    #         {   
    #             **relevant_companies_house_data,
    #             "company_check_name": company_check_name,
    #             "company_check_address": address,
    #         }, 
    #         name=company_name,
    #     )
    ...
    #         # company_data_from_company_check = pd.DataFrame([
    #         #     company_data_from_company_check\
    #         #         .append(pd.Series({**d, "sic_code": s}))
    #         #     for d, s in itertools.product(directors, sic_codes)
    #         # ])
    #         # company_data_from_company_check.index = [
    #         #     f"{company_name}_{i+1}" for i in range(company_data_from_company_check.shape[0])
    #         # ]
